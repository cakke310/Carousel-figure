...
public class ConfigConstants {
	...
	private static ImagePipelineConfig configureCaches(Context context) {
		...
		final MemoryCacheParams bitmapCacheParams = new MemoryCacheParams(
				...
				ConfigConstants.MAX_MEMORY_CACHE_SIZE / 3, // 内存缓存中准备清除但尚未被删除的总图片的最大大小,以字节为单位。
...
//								.getLayoutParams().height))// 调整大小
	...
	// 图片显示
	public static ImageRequest getScaledImageRequest(SimpleDraweeView view, String uri, int width, int height) {
		ImageRequest imageRequest = ImageRequestBuilder
				.newBuilderWithSource(Uri.parse(uri))
				.setAutoRotateEnabled(true)//自动旋转图片方向
						// .setImageDecodeOptions(getImageDecodeOptions())//  图片解码库
	...
	public static DraweeController getDraweeController(
			ImageRequest imageRequest, SimpleDraweeView view, ControllerListener<Object> listener) {
		DraweeController draweeController = Fresco.newDraweeControllerBuilder()
//				.reset()//重置
//				.setAutoPlayAnimations(true)// 自动播放图片动画
//				 .setCallerContext(callerContext)//回调
